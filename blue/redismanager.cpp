...
namespace blue
{
    ...
    {
        MRWmutex::WritelockSco lock(m_mutex);
        redisReply *reply;
        if (expire_sec > 0)
        {
    ...
    {
        MmutexType::ReadlockSco lock(m_mutex);
    ...
    {
        MmutexType::WritelockSco lock(m_mutex);
    ...
    bool RedisManager::_connect(const std::string &host, uint16_t port, const std::string &password)
    {
        ...
        {
            BLUE_LOG_DEBUGE(g_logger) << "Redis Password is empty";
        }
        return true;
    }
}